...
上传的 PDF 会经过三步清洗后才进入切片（见 `services/rag/loader.py`）：

| 步骤 | 做什么 | 为什么 |
|---|---|---|
| `_strip_running_headers` | 去掉页眉页脚 | 期刊每页都有相同的刊名与页码，混进正文既占额度又污染检索 |
| `_rejoin_wrapped_lines` | 合并排版折行 | PDF 按版面宽度折行，换行落在句子中间，不合并则切片从半句话开始 |
| `_promote_headings` | 把 `一、`、`（一）` 提升为标题 | PDF 没有标题样式，否则切片全无章节，引用卡片无法定位 |

**已知局限**：

- 折行合并是**启发式**（按行宽中位数判断），版面特殊的 PDF 可能合并不全或过度合并
- `1.` / `2.` 形式的编号**刻意不提升**为标题——实测某指南中 42 处 `1.` 大多是
  表格里的"1.诊断 2.治疗"，提升会破坏结构
- 语料里实测一份 10 页指南的章节覆盖率为 39/40（首页的标题块无章节，属正常）
