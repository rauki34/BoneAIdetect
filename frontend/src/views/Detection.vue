<template>
  <div>
    ...
    <!-- 患者选择（仅医生角色显示） -->
    <div v-if="isDoctor && file" class="patient-selection-area">
      <el-divider content-position="left">患者信息</el-divider>
      <el-form label-width="100px">
        <el-form-item label="选择患者">
          <PatientSelector v-model="selectedPatientId" @change="handlePatientChange" />
        </el-form-item>
      </el-form>
    </div>
    ...
    >
      <el-form :model="reportForm" label-width="100px">
        <el-form-item label="选择患者" required>
          <PatientSelector v-model="reportForm.patient_id" />
        </el-form-item>
        ...
        <el-form-item label="随访备注">
          <el-input
            v-model="reportForm.follow_up_notes"
            type="textarea"
            :rows="4"
            placeholder="请输入随访备注..."
          />
...
const openCreateReportDialog = () => {
  if (!result.value || !currentHistoryId.value) {
  ...
  if (selectedPatientId.value) {
    reportForm.value.patient_id = selectedPatientId.value
  }
...
const submitCreateReport = async () => {
  if (!reportForm.value.patient_id) {
  ...
  try {
    await axios.post('/api/doctor/reports', {
      ...
      follow_up_notes: reportForm.value.follow_up_notes
    })
    ...
    const errorMsg = error.response?.data?.error || '创建报告失败'
...
请用中文回复，结构化输出。`
  ...
  if (selectedPatient.value) {
    // 计算年龄
    const birthDate = selectedPatient.value.birth_date
    let age = ''
    if (birthDate) {
